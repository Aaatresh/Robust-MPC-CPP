...
/*
        This program simulates and tests a standard MPC controller on a servo-mechanical system.
            - The standard MPC controller uses a standard Kalman filter to perform state estimation in the MPC. The
              standard MPC implementation can be found in files mpc.h and mpc.cpp.
...
// Include necessary files that implement matrices, system model and the standard MPC controller.
#include "controllers/model_defs.h"             // Contains functions associated with configuration
#include "matrix.h"                 // Contains code related to matrix operations
#include "matrix.cpp"
#include "controllers/mpc.h"                    // Implements the robust MPC
#include "controllers/mpc.cpp"
#include <fstream>
#include <random>
...
// Function prototypes
void sample_gauss_pdf_iid(Matrix & , float , float , std::mt19937 );
...
int main() {
    ...
    // Initialize system with state space representative matrices. This includes a pre-processing step where
    // the continuous state space model is converted into a discrete state space model. Here, the discrete state space
    // model was found using Python and those values were used here.
    initialize_linear_system(A, B, C);

    // Create an instance of MPC type and initialize it with
    MPC controller(A, B, C, Qk, Rk, noise_var_est, P0);
    ...
    // Creating a noise vector that is used to simulate the addition of Gaussian random noise into the system
    Matrix vtt(SS_X_LEN, 1);
    ...
    for(float t = 0; t < 20; t += 0.1){
        ...
        sample_gauss_pdf_iid(vtt, 0, 1, gen);
